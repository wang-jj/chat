...
public class ChatModelImpl implements ChatModel{
    ...
    //初始化聊天
    @Override
    public void InitChat(final Callback<List<Chat>> callback){
        ThreadTask t = new ThreadTask<Void,Void,List<Chat>>(callback, new Execute<List<Chat>>(){
         @Override
         public List<Chat> doExec(){ //获取聊天记录列表List<Chat>
    ...
    public void SendMessage(final int contact_id, final String content, final int msgtype, final Callback<Void> callback){
        ...
        @Override
            public Void doExec(){ //插入Chat表，更新Message表
            String sender ;
            String receiver ;
            long time ;
            EMMessageBody body ;
            ...
            ChatDao chatDao = MyApplication.getDao().getChatDao();
            Chat chat = new Chat();
            chat.setUser_id(Integer.valueOf(sender));
            chat.setContact_id(Integer.valueOf(receiver));
            chat.setTime(time);
            ...
            Message message = messageDao.queryBuilder().where(MessageDao.Properties.User_id.eq(user_id),
                    MessageDao.Properties.Contact_id.eq(Integer.valueOf(receiver))).build().unique();
            ...
            public Void doExec(){ //插入Chat表，插入与更新Message表
                ...
                for(EMMessage msg : messages){
                    ...
                    if(themsg == null){ //在Message中创建
                        ...
                        newmsg.setContact_name("");//向服务器获取用户名
